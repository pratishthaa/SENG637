...
In order to perform white-box analysis, first we use `Eclemma`, a code coverage tool already integrated an dthus easy to use with Eclipse. We run coverage as Junit test for all the test cases we created for assignment 2, and track the different types of coverages:
  - statement coverage(mimumum 90%)
  - branch coverage(mimumum 70%)
  - method coverage(mimumum 60%)

If we get 100% coverage, we write more test cases for practice and improve at testing. Otherwise, we analyze the source code of the methods that the test cases are written for, and write more test cases for mode code coverage. And eventually calculate code coverage again to at least meet the minimum requirements.
